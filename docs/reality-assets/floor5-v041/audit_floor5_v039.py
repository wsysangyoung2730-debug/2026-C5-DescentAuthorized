...
report={'version':'v039','rooms':{},'asset_usage':{},'checks':{},'camera_checks':{}}
...
(OUT/'AUDIT_floor5_v039.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
print(json.dumps({'checks':report['checks'],'obstructions':report.get('sightline_obstructions'),'framing':report['camera_checks'],'counts':{p:r['triangles'] for p,r in report['rooms'].items()}},ensure_ascii=False))
