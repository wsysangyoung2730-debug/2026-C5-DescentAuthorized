...
extension SpellDefinition {
    ...
    var compactEffectDescription: String {
        ...
        case let .expansion(expansion):
            ...
            case .bloodSealPiercing: return "HP −\(hpCost) · 관통 \(amount)"
            case .limitBarrier: return "HP −8 · 방벽 \(amount)"
            case .executionNullification: return "HP −10 · 예약 최대 2건"
            case .directHitProhibition: return "HP −\(hpCost) · 직접 피해 1회 무효"
            case .responsibilitySeverance: return "공격 \(amount) · 모사·반격 −50%"
